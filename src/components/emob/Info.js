...
const Info = ({
  ...
  if (currentFeature) {
    ...
    subtitle = `${currentFeature.properties.strasse} ${currentFeature.properties.hausnummer}${
      currentFeature.properties.plz !== undefined ? ', ' : ''
    }${currentFeature.properties.plz || ''} ${currentFeature.properties.ort || ''}`;
  ...
  return (
    <InfoBox
      ...
      zoomToAllLabel={`${items.length} ${
        items.length === 1 ? 'Ladestation' : 'Ladestationen'
      } in Wuppertal`}
      currentlyShownCountLabel={`${featureCollection.length} ${
        featureCollection.length === 1 ? 'Ladestation' : 'Ladestationen'
      } angezeigt`}
      ...
      noCurrentFeatureContent={
        <div style={{ marginRight: 9 }}>
          <p>
            Für mehr Ladestationen Ansicht mit <Icon name="minus-square" /> verkleinern oder mit dem
