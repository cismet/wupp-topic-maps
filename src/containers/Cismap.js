...
//import gazData from '../gaz.json' //this.props.mapping.gazetteerData
...
export class Cismap_ extends React.Component {
  ...
  internalGazeteerHitTrigger(hit) {
    ...
    if (hit !== undefined && hit.length !== undefined && hit.length > 0 && hit[0].x !== undefined && hit[0].y !== undefined) {
      ...
      let zl = hitObject.more.zl;
      console.log(zl)

      this.refs.leafletMap.leafletElement.setZoom(zl, {"animate": false});
  ...
  render() {
    ...
    if (this.props.mapping.searchInProgress) {
      searchIcon = (<Icon spin="spin" name="refresh"/>)
    }
