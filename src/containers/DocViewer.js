...
export class DocViewer_ extends React.Component {
                ...
                this.props.docsActions.finished(docPackageIdParam, docIndex, pageIndex, () => {
                  setTimeout(() => {
                    if (this.props.docs.docs[docIndex] && this.props.docs.docs[docIndex].meta) {
                      ...
                      console.log('this.props.docs.docs[docIndex]', this.props.docs.docs[docIndex]);
                      console.log(
  ...
  getDocInfoWithHead = (doc, index) => {
    return fetch(
      `http://localhost:8081/rasterfariWMS?SRS=EPSG:25832&service=WMS&request=GetMap&layers=${
        doc.url
      }&styles=default&format=image%2Fpng&transparent=true&version=1.1.1&tiled=true&width=256&height=256&srs=undefined&bbox=-0.5,-0.5,0.5,0.5`,
      {
        method: 'head'
      }
    ).then(response => {
