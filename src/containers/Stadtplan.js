...
function mapStateToProps(state) {
  return {uiState: state.uiState, mapping: state.mapping, routing: state.routing, stadtplan: state.stadtplan};
}
...
export class Stadtplan_ extends React.Component {
  constructor(props, context) {
    super(props, context);
    this.gazeteerhHit = this
      .gazeteerhHit
      .bind(this);
    this.searchButtonHit = this
      .searchButtonHit
      .bind(this);
    ...
    this.gotoHome = this
      .gotoHome
      .bind(this);
    this.doubleMapClick = this
      .doubleMapClick
      .bind(this);
    this.searchTooltip = this
      .searchTooltip
      .bind(this);
    this.selectNextIndex = this
      .selectNextIndex
      .bind(this);
    this.selectPreviousIndex = this
      .selectPreviousIndex
      .bind(this);
    this.createfeatureCollectionByBoundingBox = this
      .createfeatureCollectionByBoundingBox
      .bind(this);
    ...
    this.resetFilter = this
      .resetFilter
      .bind(this);
    this.centerOnPoint = this
      .centerOnPoint
      .bind(this);
    this
      .props
      .mappingActions
      .setBoundingBoxChangedTrigger(this.createfeatureCollectionByBoundingBox);
  }
  ...
  createfeatureCollectionByBoundingBox(bbox) {
    this
      .props
      .stadtplanActions
      .createFeatureCollectionFromPOIs(bbox);
  }
  ...
  gazeteerhHit(selectedObject) {
    if (selectedObject && selectedObject[0] && selectedObject[0].more && (selectedObject[0].more.pid || selectedObject[0].more.kid)) {
      ...
      this
        .props
        .stadtplanActions
        .setSelectedPOI(selectedObject[0].more.pid || selectedObject[0].more.kid);
    }
  }
  ...
  featureClick(event) {
    if (event.target.feature) {
      this
        .props
        .mappingActions
        .setSelectedFeatureIndex(event.target.feature.index);
    }
  }
  ...
  selectNextIndex() {
    ...
    this
      .props
      .mappingActions
      .setSelectedFeatureIndex(potIndex);
  ...
  render() {
    ...
    info = (<StadtplanInfo
      ...
      showModalMenu={section => this
      .props
      .uiStateActions
      .showApplicationMenuAndActivateSection(true, section)}
    ...
    let titleContent;
    let qTitle = queryString
      .parse(this.props.routing.location.search)
      .title;
    if (qTitle !== undefined) {
      if (qTitle === null || qTitle === '') {
        if (getFilter(this.props.stadtplan).positiv.length > 0 && getFilter(this.props.stadtplan).positiv.length < getLebenslagen(this.props.stadtplan).length) {
          if (getFilter(this.props.stadtplan).positiv.length <= 4) {
            themenstadtplanDesc += getFilter(this.props.stadtplan)
              .positiv
              .join(', ');
          } else {
          ...
          if (getFilter(this.props.stadtplan).negativ.length > 0) {
            if (getFilter(this.props.stadtplan).negativ.length <= 3) {
              ...
              themenstadtplanDesc += ' (' + getFilter(this.props.stadtplan).negativ.length + ' Themen ausgeschlossen)';
            }
          }
        }
        titleContent = (
