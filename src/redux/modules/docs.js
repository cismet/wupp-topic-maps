...
function setLoadingState(loadingState) {
	return { type: types.SET_LOADING_STATE, loadingState };
}
...
//COMPLEXACTIONS
function setDelayedLoadingState() {
	return (dispatch, getState) => {
		const state = getState().docs;
		dispatch(setLoadingState(constants.LOADING_STARTED));
		setTimeout(() => {
			if (state.loadingState === constants.LOADING_STARTED) {
				dispatch(setLoadingState(constants.LOADING_OVERLAY));
			}
...
function loadPage(docPackageId, docIndex, pageIndex = 0) {
	return (dispatch, getState) => {
		...
		dispatch(setDelayedLoadingState());
		...
		if (docIndex !== state.docIndex) {
			if (pdfDocCacheHit === undefined) {
				...
				loadingTask.promise.then((pdf) => {
					...
					getPDFPage(dispatch, getState, docPackageId, docIndex, pdf, pageIndex);
				});
			} else {
				getPDFPage(dispatch, getState, docPackageId, docIndex, pdfDocCacheHit, pageIndex);
			}
		} else {
			//Same doc other page
			getPDFPage(dispatch, getState, docPackageId, docIndex, pdfDocCacheHit, pageIndex);
...
function getPDFPage(dispatch, getState, docPackageId, docIndex, pdf, pageIndex) {
	const cacheState = getState().docsCache;
