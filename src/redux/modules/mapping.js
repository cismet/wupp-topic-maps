...
function mappingBoundsChanged(bbox) {
    return {type: types.MAP_BOUNDING_BOX_CHANGED, bbox};
}

function setFeatureCollection(featureCollection) {
    return {type: types.FEATURE_COLLECTION_CHANGED, featureCollection};
}

function setSelectedFeatureIndex(index) {
    return {type: types.FEATURE_SELECTION_INDEX_CHANGED, index};
}

function setSearchProgressIndicator(inProgress) {
    return {type: types.SET_SEARCH_PROGRESS_INDICATOR, inProgress};
}

function setAutoFit(autofit, bounds, mode) {
    return {type: types.SET_AUTO_FIT, autofit, bounds, mode};
}

function gazetteerHit(hit) {
    return {type: types.GAZETTEER_HIT, hit};
}
