...
// function setPoiGazHit(hitId) {   return { type: types.SET_POI_GAZ_HIT, hitId
// }; } function clearPoiGazHit() {   return { type: types.CLEAR_POI_GAZ_HIT };
// }
...
function loadPOIs() {
  ...
  return (dispatch, getState) => {
    dispatch(dataDuck.actions.load({
...
//EXPORT SELECTORS
export const getPOIs = state => dataDuck
  .selectors
  .getItems(state.dataState);
export const getPOIGazHitId = state => state.localState.poiGazHitId;
export const getPOIsMD5 = state => dataDuck
  .selectors
  .getMD5(state.dataState);
