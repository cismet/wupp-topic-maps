...
export const getChildSVG = (svgSize = 30, bg = '#FF0000') => {
                ...
                <svg x="${svgSize / 12}" y="${svgSize / 12}"  width="${svgSize - 2 * svgSize / 12}" height="${svgSize - 2 * svgSize / 12}" viewBox="0 0 ${bdim.width} ${bdim.height || 24}">       
                    ${childSVG}
                </svg>
                </svg>  `;
...
export const getFeatureStyler = (svgSize = 30, featureRenderingOptions) => {
				return (feature) => {
                    ...
                    /* <![CDATA[ */
                        #badgefor_${feature.id} .bg-fill  {
                            fill: ${getColorForProperties(feature.properties, featureRenderingOptions)};
                        }
                        #badgefor_${feature.id} .bg-stroke  {
                            stroke: ${getColorForProperties(feature.properties, featureRenderingOptions)};
                        }
                ...
                <svg x="${selectionOffset + innerBadgeOffset}" y="${selectionOffset + innerBadgeOffset}" width="${badgeDimension}" height="${badgeDimension}" viewBox="0 0 ` + bdim.width + ` ` + bdim.height + `">
                ${badge}
...
export const getKitaClusterIconCreatorFunction = (svgSize = 24, featureRenderingOptions) => {
				// return a function because the functionCall of the iconCreateFunction cannot be
				// manipulated
				return (cluster) => {
								...
								//Kleiner Kreis in der Mitte (blau wenn selektion)
								...
								// background.appendChild(createElement('rect', {     x:0,     y:0,     width:
								// canvasSize,     height: canvasSize,     "stroke-width":1,     stroke:
								// "#000000",     opacity: "1",     fill: "#ff0000" }));
								...
								if (inCart) {
												background
																.appendChild(createElement('text', {
																x: '50%',
																y: '50%',
																'text-anchor': 'middle',
...
opts[opts.length - 1][
				kC
								.TRAEGERTYP
								.indexOf(kC.TRAEGERTYP_STAEDTISCH)
] = '#00A0B0';
opts[opts.length - 1][
				kC
								.TRAEGERTYP
								.indexOf(kC.TRAEGERTYP_ANDERE)
] = '#A1BBC1';
opts[opts.length - 1][
				kC
								.TRAEGERTYP
								.indexOf(kC.TRAEGERTYP_BETRIEBSKITA)
] = '#594F4F';
opts[opts.length - 1][
				kC
								.TRAEGERTYP
								.indexOf(kC.TRAEGERTYP_ELTERNINITIATIVE)
] = '#9DE0AD';
opts[opts.length - 1][
				kC
								.TRAEGERTYP
								.indexOf(kC.TRAEGERTYP_KATHOLISCH)
] = '#7FBCB5';
opts[opts.length - 1][
				kC
								.TRAEGERTYP
								.indexOf(kC.TRAEGERTYP_EVANGELISCH)
] = '#547980';
...
export const getDescription = (properties) => {
				switch (kitasConstants.TRAEGERTYP[properties.traegertyp]) {
								case kitasConstants.TRAEGERTYP_STAEDTISCH:
												return ('Kindertageseinrichtung mit ' + properties.plaetze + ' Plätzen in städtischer Trägerschaft');
								case kitasConstants.TRAEGERTYP_EVANGELISCH:
												return ('Kindertageseinrichtung mit ' + properties.plaetze + ' Plätzen in kirchlicher Trägerschaft (evangelisch)');
								case kitasConstants.TRAEGERTYP_KATHOLISCH:
												return ('Kindertageseinrichtung mit ' + properties.plaetze + ' Plätzen in kirchlicher Trägerschaft (katholisch)');
								case kitasConstants.TRAEGERTYP_ELTERNINITIATIVE:
												return ('Kindertageseinrichtung mit ' + properties.plaetze + ' Plätzen in Trägerschaft einer Elterninitiative');
...
export const getFilterDescription = (filter) => {
				...
				const chkProfilInklusion = filter
								.profil
								.indexOf(kitasConstants.PROFIL_INKLUSION) !== -1;
				const chkProfilNormal = filter
								.profil
								.indexOf(kitasConstants.PROFIL_NORMAL) !== -1;
				...
				if (filter.traeger.length < kitasConstants.TRAEGERTYP.length) {
								...
								if (filter.traeger.length > 3) {
												traegerDesc = '| ohne ' + Array
																.from(remainingTraeger)
												...
												traegerDesc = '| nur ' + Array
																.from(filteredTraeger)
																.map((t) => kitasConstants.TRAEGERTEXT_FOR_DESCRIPTION[t])
																.join(joiner);
								}
				}

				const radioAgeUnter2 = filter
								.alter
								.indexOf(kitasConstants.ALTER_UNTER2) !== -1;
				const radioAgeAb2 = filter
								.alter
								.indexOf(kitasConstants.ALTER_AB2) !== -1;
				...
				const chkUmfang35h = filter
								.umfang
								.indexOf(kitasConstants.STUNDEN_FILTER_35) !== -1;
				const chkUmfang45h = filter
								.umfang
								.indexOf(kitasConstants.STUNDEN_FILTER_45) !== -1;
