...
`;
const brSVG = `<svg  clip-rule="evenodd" fill-rule="evenodd" image-rendering="optimizeQuality" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" viewBox="0 0 451.56 450.43" xml:space="preserve" xmlns="http://www.w3.org/2000/svg">
<defs>
 <style type="text/css">
  <![CDATA[
...
export const addSVGToPRBR = (poi, manualReloadRequested) => {
	return new Promise(function(fulfilled, rejected) {
		...
		fetch('/svgs/' + getSignatur(poi), { method: 'get', headers: cacheHeaders })
			.then((response) => {
				if (response.ok) {
			...
			.catch(function(error) {
				console.error('Problem bei /svgs/' + getSignatur(poi), error);
				console.error(error);
